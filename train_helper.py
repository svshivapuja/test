...
class Trainer(object):
    ...
    def train(self):
        ...
        if args.resume == "":
           f = open("minibatch_train_loss.txt","w")
           f.write("")
           f.close()
           f = open("minibatch_train_mse_mae.txt","w")
           f.write("")
           f.close()
    ...
    def train_eopch(self):
        ...
        for step, (inputs, points, st_sizes, gt_discrete) in enumerate(self.dataloaders['train']):
            ...
            with torch.set_grad_enabled(True):
                ...
                #print(step,outputs.sum(1).sum(1).sum(1))
                ...
                f = open("minibatch_train_loss.txt","a+")
                f.write(str(self.epoch)+","+str(step)+","+str(ot_loss.item())+","+str(count_loss.item())+","+str(tv_loss.item())+"\n")
                f.close()    
                #print(self.tv_loss(outputs_normed, gt_discrete_normed).sum(1).sum(1).sum(1))
                #print((self.tv_loss(outputs_normed, gt_discrete_normed).sum(1).sum(1).sum(1) * torch.from_numpy(gd_count).float().to(self.device)))
                ...
                f = open("minibatch_train_mse_mae.txt","a+")
                f.write(str(self.epoch)+","+str(step)+","+str(np.mean(pred_err * pred_err))+","+str(np.mean(abs(pred_err)))+","+str(list(pred_err))+","+str(np.std(pred_err))+"\n")
                f.close()
                #print("pred",pred_err)
        self.logger.info(
        ...
        #print(epoch_loss,epoch_ot_loss,epoch_count_loss,epoch_tv_loss)
        #f = open("epoch_train_loss.txt","a+")
        #f.write(str(self.epoch)+","+str(np.mean(epoch_loss))+","+str(np.std(epoch_loss))+","+str(np.mean(epoch_ot_loss))+","+str(np.std(epoch_ot_loss))+","+str(np.mean(epoch_count_loss))+","+str(np.std(epoch_count_loss))+","+str(np.mean(epoch_tv_loss))+","+str(np.std(epoch_tv_loss))+"\n")
        #f.close()
    ...
    def val_epoch(self):
        ...
        f.write(str(self.epoch)+","+str(mae)+","+str(mse)+","+str(epoch_res)+","+str(np.std(epoch_res))+"\n")
        f.close()
